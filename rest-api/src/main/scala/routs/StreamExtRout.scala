...
class StreamExtRout(actorRef: ActorRef)  extends BaseRout {

  implicit val timeout: Timeout = 10.minute


  val getRoute = pathPrefix("lucy") {

    path("load" / RemainingPath) { path =>
      // wget "http://localhost:8112/lucy/load/fileName"
      get {
        val fileName = path.toString()
        onComplete(actorRef ? StreamTask(None, new UUID(0L, 0L), fileName, "", FileName.saveToDataBaseAndInTextFormat)) {
      ...
      uploadedFile("csv") {
        // curl --form "csv=@fileName" http://localhost:8112/lucy
        case (fileInfo, file) =>
          onComplete {
            val fileSenderActor = system.actorOf(Props[FileSenderActor])
            (fileSenderActor ? (file, new UUID(0L, 0L), fileInfo.fileName, "", FileName.saveToDataBaseAndInTextFormat)).flatMap(_ => actorRef ? FileSenderTask(fileSenderActor, fileInfo.fileName))
          }(makeResult)
      }
